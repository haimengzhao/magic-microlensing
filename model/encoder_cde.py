...
class CDEEncoder(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim, output_dim):
        ...
        # utils.init_network_weights(self.cde_func, method=nn.init.kaiming_normal_)
        ...
        # self.initial = utils.create_net(input_dim, latent_dim, n_layers=1, n_units=1024, nonlinear=nn.ReLU)
        # self.initial = nn.Sequential(
        #     utils.create_net(input_dim, latent_dim, n_layers=5, n_units=1024, nonlinear=nn.ReLU),
        #     )
        # utils.init_network_weights(self.initial, nn.init.kaiming_uniform_)
        ...
        # self.readout = nn.Sequential(
        #     utils.create_net(input_dim * 100, output_dim, n_layers=3, n_units=1024, nonlinear=nn.ReLU),
        # )
        # utils.init_network_weights(self.readout, nn.init.kaiming_normal_)
    ...
    def forward(self, coeffs):
        ...
        # plt.plot(X.evaluate(torch.linspace(X.interval[0], X.interval[-1], 125).to(coeffs.device)).detach().cpu()[0])
        # plt.show()
        ...
        z_T = z_T[:, -1]
        # z_T = X.evaluate(torch.linspace(X.interval[0], X.interval[-1], 100).to(coeffs.device)).flatten(start_dim=1)
